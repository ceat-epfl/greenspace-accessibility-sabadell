...
ORTHOPHOTO_METADATA_SHP_FILEPATH = path.join(
    ORTHOPHOTO_METADATA_DIR, "ortofoto-50cm-v7r0-metadades-20230602.shp"
)
...
rule ndvi_raster:
    input:
        spatial_extent=rule.spatial_extent.output.spatial_extent,
        orthophoto_metadata=rule.extract_orthophoto_metadata.output,
        notebook=path.join(NOTEBOOKS_DIR, NDVI_RASTER_IPYNB_FILENAME),
    params:
        stac_url="https://earth-search.aws.element84.com/v0",
        stac_collection="sentinel-2-l2a-cogs",
        stac_epsg="4326",
        max_cloud_cover=10,  # in percentage
    output:
